...
library SystemInstruction {
    ...
    /// Assign account to a program (owner)
    ///
    /// @param pda the public key for the account whose owner is going to be reassigned
    /// @param owner the public key for the new account owner
    /// @param seed PDA seed
    /// @param bump PDA bump
    function assign_pda(address pda, address owner, bytes seed, bytes bump) internal {
        AccountMeta[1] meta = [
            AccountMeta({pubkey: pda, is_signer: true, is_writable: true})
        ];
        bytes bincode = abi.encode(uint32(Instruction.Assign), owner);

        systemAddress.call{accounts: meta, seeds: [[seed, bump]]}(bincode);
    }
    ...
    /// Allocate space in a (possibly new) account without funding
    ///
    /// @param pda account for which to allocate space
    /// @param space number of bytes of memory to allocate
    /// @param seed PDA seed
    /// @param bump PDA bump
    function allocate_pda(address pda, uint64 space, bytes seed, bytes bump) internal {
        AccountMeta[1] meta = [
            AccountMeta({pubkey: pda, is_signer: true, is_writable: true})
        ];
